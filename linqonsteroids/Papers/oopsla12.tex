...
Many computations in typical programs extract information from large collections and need to do so efficiently. For persistent collections, database systems support declarative programming languages with powerful optimizers, which save much optimization effort for the programmer, while allowing queries to be specified in a compact and easy-to-maintain form. However, nowadays also in-memory collections can be quite large, especially since the amount of RAM in a typical computer is always increasing. To get good performance programmers need thus more and more to to use complex optimizations, which hinder programming and source-code maintenance. In this paper we show that many database optimizations could be adapted for this scenario and would allow developers to specify queries declaratively while retaining performance.
One common example is a source code processor analyzing the source code of a large object program, for instance to compile it or to perform static analysis oriented toward bug-finding. During execution, information on the object program is kept in memory and not in a database. Moreover, information is highly structured through object-oriented hierarchies and thus hard to map onto conventional relational databases.
%TODO elaborate, citing non-compositionality.
While object-oriented databases solve this particular problem, they are also oriented towards persistent information: in this context not the organization of data, but only execution of queries on them need to be optimized.

Typically, instead, queries are written in imperative style and optimized by hand. For instance, to make some queries faster many programs maintain dictionaries used like database indexes; manually maintaining such dictionaries in sync with the underlying data can be complex and error-prone.

A lot of research targets improved support for declarative queries, by both providing libraries and language constructs for expressing these computations and devising optimizations for queries expressed in such languages.
Typical libraries provide support for operations such as \texttt{map} and \texttt{filter} and syntactic sugar in the form of list comprehensions (like in Haskell) or SQL-like surface syntax (like in LINQ); queries written with such constructs typically require optimizations such as deforestation/fusion.
% There are many other optimizations, mention some of them.

% Mention LINQ somewhere! LINQ to Objects is a deep embedding, just a dumb one.
Modern collections libraries like the one offered by Scala can be considered domain-specific embedded languages (DSELs).
Current library designs are typically \emph{shallow} embeddings: the DSL program, here a query, is executed immediately, and optimizations are typically done at compile-time by the host language optimizer. An alternative are \emph{deep} embeddings, where the DSL program constructs an AST which can then be optimized by the library and executed. In this paper, we explore an alternative road and argue that \emph{deep} embeddings deserve more consideration. We implement a deep embedding for the core of the Scala collection library, which allows to implement powerful optimizations quite easily. To evaluate the framework, we implement a few (mostly standard) optimizations from the database world, and evaluate their effectiveness on a selection of queries.



%showing how to represent the rich complexity of its types

%Typically, modern programming languages allow specifying such computation in a compact and declarative way through analogs of list comprehensions and offer rich libraries
%as queries, 
%Higher-order operators allow formulating many such
...
%collection, because these optimizations are not supported by commonly used libraries, and performing these optimizations by hand would lead to a
%complex, error-prone and hard-to-maintain restructuring of the original
%program.
%Similarly to other functional languages, Scala allows 
%Scala offers operators like \texttt{map}, \texttt{flatMap},
...
% XXX: Only maybe we will get that.
\item
...
% Unify with below.
\item
  queries can have, as results, \lstinline!Traversable!,
  \lstinline!Seq!, \lstinline!Set! and \lstinline!Map!
% XXX (support for Seq should be possible, too);
\item
...
\item
  ...
  \lstinline!Map[A, B]!.
  %which are updated thanks to same support for IVM as
  %other query results.
  This is only possible because our data
  representation allows to also represent different kinds of collections
  uniformly.
% Drop this point for now.
%\item
...
% XXX Add more contributions.
\begin{itemize}
\item
embedded queries have precise types, just as the native Scala
collections; in particular, our embedding
supports \lstinline!Traversable!, \lstinline!Seq!, \lstinline!Set! and \lstinline!Map!;
% XXX (support for Seq should be possible, too);
%
%
% do we have this?
...
consequently, if \texttt{x: Var} then %\\ %XXX
