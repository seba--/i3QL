...
//        tmp.foreach(println _)
    ...
    private def registerFanOut(parameters : LazyView[parameter],
        ...
        val func : ((ReferenceType, Type)) => Boolean = 
            (x : (ReferenceType, Type)) => { x._1 != x._2 && x._2.isObjectType }
    ...
    private def registerFanIn(parameters : LazyView[parameter], methods : LazyView[Method], fiels : LazyView[Field]) : LazyView[(Type, Int)] = {
        val func : ((ReferenceType, Type)) => Boolean = (x : (ReferenceType, Type)) => { x._1 != x._2 && x._2.isObjectType }
...
//        
//        val rec = new WithRecursiveImpl(extendz, (x : `extends`) => (x.source,x.target), (a : `extends`, b : (ObjectType,ObjectType)) => ({
//           if(a.target == b._1)
//               Some((a.source,b._2))
//           else
//               None
