...
public class FullTrieInspector {

	private static final PostgreSQL DB = new PostgreSQL();
	
	public FullTrieInspector() {
		DB.connect();
	}
	
	public void inspect(FullTrie root, FullTrieBuilder builder) {
		System.out.println("Deleting old results from DB...");
		DB.modify("DELETE FROM trie_nodes");

		Iterator<FullTrie> iter = builder.nodeIterator(root);
	...
	private void collectData(FullTrie node) {
