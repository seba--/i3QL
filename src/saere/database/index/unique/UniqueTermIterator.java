 ...
 */
public class UniqueTermIterator extends UniqueIteratorBase implements Iterator<Term> {
	
	/** The next term. It is set by {@link UniqueQueryTrieIterator#findNext()} (only). */
	protected Term next;
	
	/**
	 * Creates a new simple trie iterator <b>without intialization</b> (the first element is not found).
	 * @see UniqueIteratorBase#BaseTrieIterator()
	 */
	protected UniqueTermIterator() { /* empty */ }
	 ...
	 */
	protected UniqueTermIterator(UniqueTrie start) {
		super(start);
		init();
	}
	
	/**
	 * Initializes this iterator (used in the constructor and {@link #resetTo(UniqueTrie)}).
	 */
	private void init() {
