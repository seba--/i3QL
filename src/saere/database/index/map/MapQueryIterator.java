 ...
 * {@link MapTrie} class is used.<br/>
 * <br/>
 * <b>This iterator works only with {@link MapTrie}s that have been built with 
 * a {@link SimpleTermInserter}.</b>
 * 
 * @author David Sullivan
 * @version 0.1, 10/26/2010
 */
public class MapQueryIterator extends MapTermIterator implements Iterator<Term> {
	
	private final MapTermIterator subiterator;
	 ...
	 */
	public MapQueryIterator(MapTrie start, QueryStack stack) {
		this.start = start;
		current = start;
		
		// create the one and only instance of the subiterator that'll be used
		subiterator = new MapTermIterator(start);
		...
		while (next == null && current != null) { // (or break)
			...
			if (useSubiterator) { // subiteration mode
				...
				if (match) {
					...
					MapTrie searched = current.parent.getChild(label);
