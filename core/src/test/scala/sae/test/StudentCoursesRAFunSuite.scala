    ...
    extends FunSuite
{
    ...
    test ("joins") {
        ...
        val course_for_student = ((students, students.Id) ⋈ (enrollments.StudentId, enrollments)) {
            (s: Student,
             e: Enrollment) => (s, e)
        }
    ...
    test ("equi join") {
        ...
        */
        // version with double parameter infix on key + relation
            ((students, students.Id) ⋈ (enrollments.StudentId, enrollments)) {
        ...
        val student_courses = ((student_names_to_courseId, (_: (Student, Integer))._2) ⋈ (courses
            .Id, courses))
        {
            (e: (Student, Integer), c: Course) => (e._1, c)
        }
    ...
    test ("removal from equi join") {
        ...
        val theEasterBunny = new Person {
            val Name = "Easter Bunny"
        }
        students += first
        students += second
        students += third
    ...
    test ("semi join as differences") {
        ...
        {
            (left: Student, right: Person) => left
        }
    ...
    test ("aggragate sort") {
        ...
        val agg: Relation[(String, Seq[Integer])] = γ (students,
            (_: Student).Name,
            Sort ((_: Student).Id),
            (key: String, value: SortedMultiset[Integer]) => (key, JavaConverters.asScalaIteratorConverter(value.iterator()).asScala.toList)
        )

        Assert.assertEquals (
            List (
                (john.Name, Seq (john.Id)),
                (sally.Name, Seq (sally.Id))
            ),
            agg.asList.sortBy(_._1)
        )

        val otherJohn = Student (01234, "john")
        val otherSally = Student (99999, "sally")
        students.element_added (otherJohn)

        Assert.assertEquals (
            List (
                (john.Name, Seq (otherJohn.Id, john.Id)),
                (sally.Name, Seq (sally.Id))
            ),
            agg.asList.sortBy(_._1)
        )

        students.element_added (otherSally)

        Assert.assertEquals (
            List (
                (john.Name, Seq (otherJohn.Id, john.Id)),
                (sally.Name, Seq (sally.Id, otherSally.Id))
            ),
            agg.asList.sortBy(_._1)
        )

        students.element_removed (john)

        Assert.assertEquals (
            List (
                (john.Name, Seq (otherJohn.Id)),
                (sally.Name, Seq (sally.Id, otherSally.Id))
            ),
            agg.asList.sortBy(_._1)
        )

        students.element_removed (otherJohn)
