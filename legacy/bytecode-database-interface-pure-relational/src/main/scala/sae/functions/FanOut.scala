...
private class FanOutIntern[Domain <: AnyRef](val f: Domain => (Seq[Type], Type),
                                             val select: Type => Boolean) extends SelfMaintainableAggregateFunction[Domain, Set[String]]
{
...
object FanOut
{
    def apply[Domain <: AnyRef](f: (Domain => (Seq[Type], Type)), select: Type => Boolean) = {
        new SelfMaintainableAggregateFunctionFactory[Domain, Set[String]]
        {
            def apply(): SelfMaintainableAggregateFunction[Domain, Set[String]] = {
