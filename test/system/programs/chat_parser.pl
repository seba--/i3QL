...
input([does,afghanistan,border,china,?]).
input([what,is,the,capital,of,upper_volta,?]).
input([where,is,the,largest,country,?]).
input([which,country,'`',s,capital,is,london,?]).
input([which,countries,are,european,?]).
input([how,large,is,the,smallest,american,country,?]).
input([what,is,the,ocean,that,borders,african,countries,
	and,that,borders,asian,countries,?]).
input([what,are,the,capitals,of,the,countries,bordering,the,baltic,?]).
input([which,countries,are,bordered,by,two,seas,?]).
input([how,many,countries,does,the,danube,flow,through,?]).
input([what,is,the,total,area,of,countries,south,of,the,equator,
	and,not,in,australasia,?]).
input([what,is,the,average,area,of,the,countries,in,each,continent,?]).
input([is,there,more,than,one,country,in,each,continent,?]).
input([is,there,some,ocean,that,does,not,border,any,country,?]).
input([what,are,the,countries,from,which,a,river,flows,
	into,the,black_sea,?]).
...
is_pp(comment(1,_,_,_)).

is_pred(comment(_,1,_,_)).

is_trace(comment(_,_,1,_)).

is_adv(comment(_,_,_,1)).

trace(comment(_,_,1,_),comment(0,0,0,0)).

trace(comment(0,0,1,0)).

adv(comment(0,0,0,1)).

empty(comment(0,0,0,0)).

np_all(comment(1,1,1,0)).

s_all(comment(1,0,1,1)).

np_no_trace(comment(1,1,0,0)).

% mask operations

myplus(comment(B1,B2,B3,B4),comment(C1,C2,C3,C4),comment(D1,D2,D3,D4)) :-
   or(B1,C1,D1),
   or(B2,C2,D2),
   or(B3,C3,D3),
   or(B4,C4,D4).

minus(comment(B1,B2,B3,B4),comment(C1,C2,C3,C4),comment(D1,D2,D3,D4)) :-
   anot(B1,C1,D1),
   anot(B2,C2,D2),
   anot(B3,C3,D3),
   anot(B4,C4,D4).
...
role(subj,_,comment(1,0,0)).
role(compl,_,comment(0,_,_)).
role(undef,main,comment(_,0,_)).
role(undef,aux,comment(0,_,_)).
role(undef,decl,_).
role(nil,_,_).

subj_case(comment(1,0,0)).
verb_case(comment(0,1,0)).
prep_case(comment(0,0,1)).
compl_case(comment(0,_,_)).
...
quant(B,indef,C,D,E,F) :-
   neg_adv(G,B,C,H,E,I),
   comp_adv(G,H,J,I,K),
   backtick(than,J,D,K,F).
quant(B,indef,C,D,E,F) :-
   backtick(at,C,G,E,H),
   sup_adv(I,G,D,H,F),
   sup_op(I,B).
quant(the,def,B,C,D,E) :-
   backtick(the,B,C,D,E).
...
comp(B,C,D,E,F,G) :-
   comp_adv(B,D,H,F,I),
   adj(quant,C,H,J,I,K),
   backtick(than,J,E,K,G).
comp(more,B,C,D,E,F) :-
   rel_adj(B,C,G,E,H),
   backtick(than,G,D,H,F).
comp(same,B,C,D,E,F) :-
   backtick(as,C,G,E,H),
   adj(quant,B,G,I,H,J),
   backtick(as,I,D,J,F).
...
comp_adv(less,B,C,D,E) :-
   backtick(less,B,C,D,E).
comp_adv(more,B,C,D,E) :-
   backtick(more,B,C,D,E).


sup_adv(least,B,C,D,E) :-
   backtick(least,B,C,D,E).
sup_adv(most,B,C,D,E) :-
   backtick(most,B,C,D,E).
...
int_art(B,plu,quant(same,wh(B)),C,D,E,F) :-
   backtick(how,C,G,E,H),
   backtick(many,G,D,H,F).
...
context_pron(prep(in),place,B,C,D,E) :-
   backtick(where,B,C,D,E).
context_pron(prep(at),time,B,C,D,E) :-
   backtick(when,B,C,D,E).
...
backtick(there).
backtick(of).
backtick('`').		% use ` instead of ' to help assembler
backtick(',').	
backtick(s).
backtick(than).
backtick(at).
