...
wff(implies(and(implies(X,Y),
                and(implies(Y,Z),
                    and(implies(Z,U),
                        implies(U,W)))),
            implies(X,W))) :-
        X = f(plus1(plus1(a,b),plus1(c,zero))),
        Y = f(times(times(a,b),plus1(c,d))),
        Z = f(reverse(append(append(a,b),[]))),
        U = equal(plus1(a,b),difference(x,y)),
...
difference(X, X, zero) :- !.
difference(plus1(X,Y), X, fix(Y)) :- !.
difference(plus1(Y,X), X, fix(Y)) :- !.
difference(plus1(X,Y), plus1(X,Z), difference(Y,Z)) :- !.
difference(plus1(B,plus1(A,C)), A, plus1(B,C)) :- !.
difference(add1(plus1(Y,Z)), Z, add1(Y)) :- !.
difference(add1(add1(X)), 2, fix(X)).

eq(plus1(A,B), zero, and(zerop(A),zerop(B))) :- !.
eq(plus1(A,B), plus1(A,C), equal(fix(B),fix(C))) :- !.
...
exp(I, plus1(J,K), times(exp(I,J),exp(I,K))) :- !.
...
quotient(plus1(X,plus1(X,Y)),2,plus1(X,quotient(Y,2))).
...
times(X,         plus1(Y,Z),      plus1(times(X,Y),times(X,Z))      ) :- !.
