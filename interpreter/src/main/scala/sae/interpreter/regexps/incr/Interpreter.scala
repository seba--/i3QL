...
/*	def interp(e : Exp, c : Value): Value = e match {
	...
	def interpNorm(e : (Exp, Text)) : InterpValue = {
		if (e._1.isInstanceOf[Terminal]) {
			val t1 : Terminal = e._1.asInstanceOf[Terminal]
			val res : InterpValue = if (e._2.startsWith(t1.s)) Set(e._2.substring(t1.s.length)) else Set()
			return res
		} else if (e._1.isInstanceOf[Alt]) {
			val t1 : Alt = e._1.asInstanceOf[Alt]
			...
			val res : InterpValue = Set(e._2) ++ v1
			return res
		} else if (e._1.isInstanceOf[Sequence]) {
			val t1 : Sequence = e._1.asInstanceOf[Sequence]
...
/*	def interpNorm(e : (Exp, Text)) : Value = {
	...
	type IStringListTable = KeyMapTable[StringListKey, StringList]

	type ITable = (IInputTable, ITaskTable, IExpKindTable, IStringListTable)
	...
	def addInput(exp : Exp, text : Text, tab : ITable) : TaskKey = {
		...
		val strKey = insertString(text,tab)
		val inKey = inputTable(tab).add((expKey, strKey))
	...
	private def insertString(s : String, tab : ITable) : StringListKey = {
		if (s.isEmpty)
			stringListTable(tab).add((0, -1))
		else
			stringListTable(tab).add((s.charAt(0),insertString(s.substring(1),tab)))
	}
	...
	def getValues(tab : ITable) : PartialFunction[TaskKey,InterpValue] with Iterable[(TaskKey,InterpValue)] = {
	...
	def createIExp : ITable = (
		...
		new KeyMapTable[StringListKey, StringList](new IntKeyGenerator)
	)
	...
	private class IncrementalInterpreter(val tab : ITable) {
		...
		protected val createValueTerminal : Rep[((ExpNode, Text, Seq[InterpValue])) => InterpValue] = staticData (
			(t : (ExpNode, Text, Seq[InterpValue])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[InterpValue] = t._3

				val param0 : String = e._3(0).asInstanceOf[String]
				val res : InterpValue = if (c.startsWith(param0)) scala.collection.immutable.Set(c.substring(param0.length)) else scala.collection.immutable.Set()
				res
			}
		)

		protected val createValueAlt : Rep[((ExpNode, Text, Seq[InterpValue])) => InterpValue] = staticData (
			(t : (ExpNode, Text, Seq[InterpValue])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[InterpValue] = t._3

				val value0: InterpValue = values(0)
				val value1: InterpValue = values(1)
				val res: InterpValue = value0 ++ value1
				res
			}
		)

		protected val createValueAsterisk : Rep[((ExpNode, Text, Seq[InterpValue])) => InterpValue] = staticData (
			(t : (ExpNode, Text, Seq[InterpValue])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[InterpValue] = t._3
				...
				val value1: InterpValue = values(0)
				val res: InterpValue = scala.collection.immutable.Set(c) ++ value1
				res
			}
		)

		protected val createValueSequence : Rep[((ExpNode, Text, Seq[InterpValue])) => InterpValue] = staticData (
			(t : (ExpNode, Text, Seq[InterpValue])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[InterpValue] = t._3
		...
		protected val conditionAsterisk : Rep[((IInput, IExp, ITask)) => Boolean] = staticData (
			(t : (IInput, IExp, ITask)) => t._2._2._1 == AsteriskKind
		)
	...
	//	private def taskGetText(t : Rep[ITask]) : Rep[Text] = t._2._4
		...
		private def inputGetStringListKey(t : Rep[IInput]) : Rep[StringListKey] = t._2._2
		...
		private def valGetValue(v : Rep[IValue]) : Rep[InterpValue] = v._2
		...
		private val createInput : Rep[((ExpKey, Text)) => InputKey] = staticData (
			(e : ((ExpKey, Text))) => inputTable(tab).add(e)
		)

		private val createSequenceExpression : Rep[((Seq[ExpKey], Seq[Any])) => ExpKey] = staticData (
			(e : ((Seq[ExpKey], Seq[Any]))) => expKindTable(tab).add((SequenceKind, e._1, e._2))
		)
		...
		protected def makeValue(k : Rep[TaskKey], v : Rep[InterpValue]) : Rep[RecType] =
			valueToRecType((k,v))
				...
				(rec:  Rep[Query[RecType]]) => {
					...
					recursionBase UNION ALL (
						unionPrivate (
							...
							SELECT (
								(in : Rep[IInput], e : Rep[IExp], t : Rep[ITask]) =>
									makeValue(taskGetId(t), createValueTerminal(expGetExp(e), inputGetText(in), Seq()))
							) FROM (
								inputRel, expressionRel, taskRel
							) WHERE (
								(in : Rep[IInput], e : Rep[IExp], t : Rep[ITask]) =>
									conditionTerminal(e) AND
						...
						,
							...
							SELECT (
								(in : Rep[IInput], e: Rep[IExp], childVals : Rep[(IValue, IValue)], tasks : Rep[(ITask, ITask, ITask)]) =>
									makeValue(taskGetId(tasks._1), createValueAlt(expGetExp(e), inputGetText(in), Seq(childVals._1._2, childVals._2._2)))
							) FROM (
						...
						,
							...
							SELECT (
								(in : Rep[IInput], e: Rep[IExp], parent: Rep[IExp], parentTask : Rep[ITask]) =>
									newTask("Ast1")(taskGetId(parentTask), ZERO, createInput(createSequenceExpression(Seq(e._1, parent._1), Seq()), inputGetText(in)))
							) FROM (
								inputRel, expressionRel, expressionRel, taskRel
							) WHERE (
								(in : Rep[IInput], e: Rep[IExp], parent: Rep[IExp], parentTask : Rep[ITask]) =>
									conditionAsterisk((in, parent, parentTask)) AND
									inputGetInputKey(in) == taskGetInputKey(parentTask) AND
									expGetExpKey(parent) == inputGetExpKey(in) AND
									expGetExpKey(e) == expGetChildren(parent)(0)
							)
						,
							...
							SELECT (
								(in : Rep[IInput], e : Rep[IExp], childVal : Rep[IValue], tasks : Rep[(ITask, ITask)]) =>
									makeValue(taskGetId(tasks._1),createValueAsterisk(expGetExp(e), inputGetText(in), Seq(valGetValue(childVal))))
							) FROM (
								inputRel, expressionRel, valueRel, SELECT(*) FROM (taskRel, taskRel)
							) WHERE (
								(in : Rep[IInput], e : Rep[IExp], childVal : Rep[IValue], tasks : Rep[(ITask, ITask)]) =>
									conditionAsterisk((in,e,tasks._1)) AND
									inputGetInputKey(in) == taskGetInputKey(tasks._1) AND
									expGetExpKey(e) == inputGetExpKey(in) AND
									valGetId(childVal) == taskGetId(tasks._2) AND
									taskGetParentId(tasks._2) == taskGetId(tasks._1) AND
									taskGetIndex(tasks._2) == ZERO
							)
						,
		...
		val result = new MaterializedMap[TaskKey, InterpValue]

		values.addObserver(result)


		//String Queries
		def iStringListLength(q : Rep[Query[IStringList]], key : Rep[StringListKey]) : Rep[Int] = {
			val startElements : Rep[Query[IStringList]] =
				SELECT (*) FROM (q) WHERE ((s : IStringList) => key == s._1)

			val computation =
				WITH RECURSIVE (
					(rec:  Rep[Query[RecType]]) => {
						startElements UNION ALL (
							SELECT (*) FROM (rec, q) WHERE ((s1 : IStringList, s2 : IStringList) => s1._2._2 == s2._1)
						)
					}
				)

			val sum = (SELECT (COUNT(*)) FROM computation).asList(0)
