...
//			val f : Function[String,Value] = s => interpNorm((t1.r2,s))
	...
	private class IncrementalInterpreter(val tab : ITable) {
		...
		protected val createValueTerminal : Rep[((ExpNode, Text, Seq[Value])) => Value] = staticData (
			(t : (ExpNode, Text, Seq[Value])) => {
				...
				val res : Value = if (c.startsWith(param0)) scala.collection.immutable.Set(c.substring(param0.length)) else scala.collection.immutable.Set()
				res
			}
		)

		protected val createValueAlt : Rep[((ExpNode, Text, Seq[Value])) => Value] = staticData (
			(t : (ExpNode, Text, Seq[Value])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[Value] = t._3
		...
		protected val createValueAsterisk : Rep[((ExpNode, Text, Seq[Value])) => Value] = staticData (
			(t : (ExpNode, Text, Seq[Value])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[Value] = t._3
				...
				val res: Value = scala.collection.immutable.Set(c) ++ value1
				res
			}
		)

		protected val createValueSequence : Rep[((ExpNode, Text, Seq[Value])) => Value] = staticData (
			(t : (ExpNode, Text, Seq[Value])) => {
				val e : ExpNode = t._1
				val c : Text = t._2
				val values : Seq[Value] = t._3
